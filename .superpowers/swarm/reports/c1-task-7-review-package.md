...
?? .superpowers/swarm/reports/c1-task-7-full-prefill-red.md
?? .superpowers/swarm/reports/c1-task-7-full-prefill.md
?? docs.zip
?? docs/research/
?? docs/archive/tasks/amdev-doorbell-delivery/phase-9-cp-mec-rs64-handoff.md
...
+|---|---|---|---|---|---|
+| C1PrefillRed | C1-7 RED | Focused prefill tests and validation-command row | C1-6 Done | `.superpowers/swarm/reports/c1-task-7-full-prefill-red.md` | Planned |
+| C1PrefillImpl | C1-7 GREEN | `native_r9700/prefill.py` full 16-layer prompt-0 prefix prefill | RED observed | `.superpowers/swarm/reports/c1-task-7-full-prefill.md` | Planned |
+| C1PrefillReview | C1-7 review | Task-scoped correctness/quality review | GREEN verified | `.superpowers/swarm/reports/c1-task-7-full-prefill-review.md` | Planned |
+
+### Supervisor gates
+- Report checks: RED/implementation/review reports agree on prompt-0 full-layer K/V scope, explicit longer-prompt status, no tinygrad, and no fake Qwen support.
+- Quality bar: correctness via fixture deltas and layer-order checks; maintainability via one narrow prefill module; architectural fit via existing C1 contracts and task-set-8 handoff arrays; simplicity via no generic model runner/target registry.
+- Verification command(s) supervisor will run: focused C1-7 RED/GREEN pytest command recorded in `validation-commands.md`, CLI smoke writing a local log and candidate `.npz`, combined `tests/native_r9700 -v`, full `tests -v`, and `git diff --check`.
+- Ledger update: C1-7 In progress while RED/GREEN/review run; dependent C1-8/9 remain blocked until C1-7 review/verification passes.
...
+| C1-7. Full layer stack prefill path | In progress | Main / C1Prefill | C1-5, C1-6 | `.superpowers/swarm/reports/c1-task-7-full-prefill.md` | Unblocked by C1-6 reviewed/verified layer0 K/V writer. Wave 4 scope: narrow NumPy/stdlib+safetensors Llama 3.2 1B prefix prefill through all 16 layers for prompt-0 first, producing ordered per-layer K/V arrays for task set 8; Qwen remains deferred/unsupported for this C1 Llama ladder. | |
...
@@ -418,6 +418,26 @@ Observed: exit `0`; log includes `layer=0`, `n_prefix=5`, K/V max/mean deltas
...
+fails with a clear missing `native_r9700.prefill` module/API failure. The
+model-backed tests skip only when the local Llama MLX model path or committed
+`tests/native_r9700/fixtures/kv_state.npz` fixture is absent.
+
...
## File: `.superpowers/swarm/reports/c1-task-7-full-prefill-red.md`
...
## File: `.superpowers/swarm/reports/c1-task-7-full-prefill.md`
...
- CLI: `python -m native_r9700.prefill --model <model_dir> --fixtures-dir tests/native_r9700/fixtures --prompt-name prompt-0 --out <path.npz> --log <path.log>` loads `prompts.json`, splits S-1/final token via `attention.split_prompt_tokens_for_cache`, runs full prefill, writes the NPZ, compares layer 0 and layer 15 to `kv_state.npz` when present, prints compact reports, and writes `command`, `model`, `config`, `prompt`, `final_token_id`, `n_prefix`, `num_layers`, `output`, `deltas`, and `exit_status` to the log.
...
This file is the shared command ledger for `docs/archive/tasks/native-r9700-producer/`. Agents must add exact commands here when a phase task discovers them. Do not write placeholder commands; if a command is not knowable before implementation, name the task set that must discover it.
...
Phase doc: `docs/archive/tasks/tinygrad-kv-worker/phase-0-parity.md`
...
| C0B-4.5. gfx12 VM/PTE/TLB prerequisite | Done | Main / C0BVmRedContract / C0BVmSelfTests / C0BVmHardwareMapping / C0BVmPhase2Reviewer / C0BVmFinalReviewer | C0B-4, C0B-5 blocker | `docs/archive/superpowers/plans/2026-08-16-gfx12-vm-pte-tlb-mapping.md`; `docs/archive/tasks/native-r9700-gfx12-vm-pte-tlb/README.md`; `.superpowers/swarm/reports/c0b-vm-task-1-contracts.md`; `.superpowers/swarm/reports/c0b-vm-task-2-selftests.md`; `.superpowers/swarm/reports/c0b-vm-phase1-review.md`; `.superpowers/swarm/reports/c0b-vm-task-3-hardware-mapping.md`; `.superpowers/swarm/reports/c0b-vm-phase2-review.md`; `.superpowers/swarm/reports/c0b-vm-task-4-transfer-resume.md`; `.superpowers/swarm/reports/c0b-vm-final-review.md` | Phase 1 reviewed: RED focused pytest exited `1` on absent VM self-tests; GREEN focused pytest reported `8 passed in 4.87s`; reviewer found no findings. Phase 2 implemented fixed IP discovery/register/MMIO/page-table/MMHUB VMID0/TLB setup; final supervisor pytest passed `8 passed in 6.54s`; final hardware transfer command at that time exited `1` at precise post-VM `failure_stage: sdma_ring_setup`; phase 2 and final reviewers accepted with no Critical/Important/Minor findings. | VM/PTE/TLB prerequisite is complete; C0B-5 was later completed by the SDMA queue0 transfer proof. |
