...
| Wave A / F1 persistent warm worker | Blocked | Supervisor integration | B0 | `f1-contract-freeze.md`, F1 RED/GREEN reports | Task sets 1–4 implemented; subsystem gate passes 313 tests. A real process smoke built and launched the branch runner, closed runner-identity/pack-order/private-stdout defects, and reached native resident allocation. | Hard blocker: `Resident GPU virtual address range is exhausted`. The current fixed PDB1/PDB0 DynamicPageTable covers at most 1 GiB (and the large-BAR layout exposed ~2 MiB), below full resident Llama weights. Multi-PDB1 ownership must land before the real ten-sample smoke can pass. |
...
| Wave A / Q1 Qwen contract/oracle | Needs review | Q1Acceptance | B0 | `q1-identity-freeze.md`, `q1-tensor-inventory.md`, `q1-hybrid-cache-green.md`, `q1-oracle-fixtures.md`, `q1-native-shape-map.md`, `q1-acceptance-package.md` | Task sets 1–3 are complete with 41 tensor and 46 hybrid contract tests passed; supervisor recorded 146 oracle tests passed, fixture generation passed, and oracle parity passed. Task sets 4–6 are Needs review pending final reviewer; package identity matches source pin, inventory, and fixture schema. | Q1/F6 promotion remains blocked by `base_model_revision=unavailable_in_pinned_conversion_metadata`; Q1 supplies no native/performance evidence. |
