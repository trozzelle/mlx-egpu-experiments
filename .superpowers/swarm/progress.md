...
| Wave A / P1 TinyGPU Device Owner | In progress | P1 source integration | B0, ADR 0007 | `p1-abi-freeze.md`; `p1-xcode-resume.md`; P1 RED/fix/review reports | Task set 1 Done. Xcode 26.6/DriverKit 25.5 selected. Task-set-2 source/package/common-client and task-set-3 bounded resource/token foundations compile, pass host contracts, and pass code/security re-review with zero Critical/Important findings; task-set-3 controls: 62 passed. | Task-set-2 hardware acceptance is blocked on provenance-bound PSP/SOS/TMR firmware/transitions and signed install evidence; task set 3 still needs actual DriverKit BO/import/VA, selectors, cleanup, and `client-death`; task 4 waits for those handles; task 6/promotion also requires G0. |
