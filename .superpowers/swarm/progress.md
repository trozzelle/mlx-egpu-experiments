...
| Wave A / P1 TinyGPU Device Owner | In progress | P1 source integration | B0, ADR 0007 | `p1-abi-freeze.md`; `p1-xcode-resume.md`; P1 RED/fix/review/integration reports | Task set 1 Done. Task-set-2 safe source/package/common-client boundary compiles. Task-set-3 now has reviewed bounded ownership/token/request/transport cores plus real type-0 host-visible allocation/release, ordered close cleanup, and exact `client-death` source command. Nine host contracts and both unsigned Xcode targets pass; task-set-3 controls: 62 passed; final focused reviews have zero Critical/Important findings. | Task-set-2 hardware acceptance lacks provenance-bound PSP/SOS/TMR transitions and signed install evidence. Task set 3 still lacks real descriptor import, device-local VRAM, and AMD private-VM PTE map/unmap; task 4 waits for those mappings; task 6/promotion also requires G0. |
