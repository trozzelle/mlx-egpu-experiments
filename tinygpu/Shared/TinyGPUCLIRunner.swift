...
final class TinyGPUCLIRunner: NSObject, OSSystemExtensionRequestDelegate {
  ...
  static func statusText(_ state: DextState) -> String {
    ...
    case .activated: return "DriverKit extension is active; device health is unchecked until a structured health query runs.\n\n"
    }
  }
  ...
  func request(_ request: OSSystemExtensionRequest, didFinishWithResult result: OSSystemExtensionRequest.Result) {
    switch result {
    case .completed:
      print("Driver extension \(isInstall ? "activated" : "deactivated") successfully; device health is unchecked until a structured health query runs.\n")
