...
This C0A task set 4 command is retained only as historical context. Do **not** use the old C0A22 `readback_mismatch`/`compute_output_readback_byte_swap` wording as current guidance or a blocker. The current reviewed hardware result is the C0A25 load-path value-lane fix: `logs/c0p-native-amdev-kernel-load-fix.log` records `KERNEL_PROOF_PASS`, `kernel_blob_load_status: pass`, `kernarg_write_status: pass`, `sdma_h2d_status: pass`, `sdma_d2h_status: pass`, `kernel_launch_status: pass`, `cpu_comparison_status: pass`, `host_device_transfer_status: pass`, `failure_stage: none`, `failure_text: none`, and `exit_status: 0`. C1R-2 wraps that frozen C0A25 probe through `native_r9700.runtime.RuntimeSession::kernel_proof`; the reviewed wrapper proof is `logs/c1-runner-kernel-proof-2026-08-19T11:09:34Z.log` with `kernel_proof_wrapper_status: pass` and `wrapper_exit_status: 0`.
...
| C1 | native runtime shell validation command | `phase-c1-native-producer-parity.md` task set 1 or 4 | C0 substrate SELECTED; C1 runtime contract accepted from C0/C1 validation, no separate C1 hardware runner required before model parity |
| C1 | primitive kernel test commands | `phase-c1-native-producer-parity.md` task set 5 | Exact focused command recorded below; C1 primitives suite is green |
| C1 | attention/RoPE/KV writer test command | `phase-c1-native-producer-parity.md` task set 6 | Exact focused RED/GREEN command recorded below; C1 attention suite is green |
| C1 | CPU-reference full-stack prefill smoke command | `phase-c1-native-producer-parity.md` task set 7 | Exact focused RED/GREEN and request-token smoke commands recorded below; CPU-reference prefill suite is green; Native R9700 C1 remains open until model-forward tensor work runs on the R9700/eGPU |
| C1 | native KV emitter/load round-trip command | `phase-c1-native-producer-parity.md` task set 8 | Exact focused RED/GREEN and mlx-lm load smoke commands recorded below; C1 KV emitter suite is green |
| C1 | CPU-reference producer parity command | `phase-c1-native-producer-parity.md` task set 9 | Exact reference/ABI parity command recorded below; observed `C1 parity gate_result=pass prompts=3`, but this is not Native R9700 acceptance (ADR 0005) |
| C2 | mlx-lm wrapper focused test command | `phase-c2-serving-integration.md` task set 1 or 2 | Discovered by C2 task set 1 and recorded below: `python -m pytest tests/native_r9700/test_serving.py -v`; expected RED until `native_r9700.serving` exists |
| C2 | fallback/error-state test command | `phase-c2-serving-integration.md` task set 1 or 3 | Same focused C2 serving test command covers below-threshold fallback, producer-unavailable fallback, malformed cache-before-acceptance fallback, and no post-acceptance recompute |
| C2 | serving integration command | `phase-c2-serving-integration.md` task set 4 | Discovered by C2 task set 1 and recorded below: `python -m native_r9700.serving ... --threshold-tokens 128 --artifacts-dir logs/c2-serving --json logs/c2-serving/result.json --log logs/c2-serving/run.log --report docs/path-a-validation-results.md` |
...
Qwen3.8-27B target decision coverage:
...
Observed after adding explicit Qwen negative coverage: command exits `0` with
**20 passed**. `test_qwen3vl_target_is_rejected_as_unsupported_for_c1` covers a
representative local Qwen3VL/4-bit config (`model_type=qwen3_5`,
`Qwen3_5ForConditionalGeneration`, VLM processor, 4-bit affine quantization,
64-layer hybrid linear/full attention, mRoPE) and asserts the C1 loader rejects
it as unsupported for the Llama-3.2-1B C1 parity gate.


### C1 native runner runtime shell (Lane A — task set 4)

The `native_r9700` runner shell (`native_r9700/runtime.h`, `runtime.cpp`,
`runtime_contract.cpp`, `runner.cpp`) refactors the proven C0 probe lifecycle into a narrow,
...
encodings, and standardized log writing under `logs/` — no TinyGPU socket. Its
hardware modes wrap the frozen C0A25 proof (`--kernel-proof`,
`producer_kind: hardware_probe`) and the C1R-4 streaming transfer bridge
(`--transfer-proof`, `producer_kind: hardware_memory_transfer`).
...
xcrun --sdk macosx clang++ -std=c++17 -O2 -Wall -Wextra \
  native_r9700/runtime_contract.cpp native_r9700/amdev_packets.cpp native_r9700/amdev_session.cpp native_r9700/device_memory.cpp native_r9700/runtime.cpp native_r9700/runner.cpp \
...
{ printf "%s\n" "command: xcrun --sdk macosx clang++ -std=c++17 -O2 -Wall -Wextra native_r9700/runtime_contract.cpp native_r9700/amdev_packets.cpp native_r9700/amdev_session.cpp native_r9700/device_memory.cpp native_r9700/runtime.cpp native_r9700/runner.cpp -o build/native-r9700-runtime/native_r9700_runner && build/native-r9700-runtime/native_r9700_runner --lifecycle-dry-run"; date -u "+timestamp_utc: %Y-%m-%dT%H:%M:%SZ"; xcrun --sdk macosx clang++ -std=c++17 -O2 -Wall -Wextra native_r9700/runtime_contract.cpp native_r9700/amdev_packets.cpp native_r9700/amdev_session.cpp native_r9700/device_memory.cpp native_r9700/runtime.cpp native_r9700/runner.cpp -o build/native-r9700-runtime/native_r9700_runner && build/native-r9700-runtime/native_r9700_runner --lifecycle-dry-run; status=$?; printf "wrapper_exit_status: %d\n" "$status"; exit "$status"; } 2>&1 | tee "$log"
```
...
`lifecycle_skip_rejected: yes`, and a timestamped log under `logs/`.

Runtime shell hardware proof command:
...
Observed 2026-08-19: command exited `0` and wrote
`logs/c1-runner-kernel-proof-2026-08-19T11:09:34Z.log` with
`producer_kind: hardware_probe`, `kernel_blob_load_status: pass`,
`kernarg_write_status: pass`, `sdma_h2d_status: pass`, `sdma_d2h_status: pass`,
`kernel_launch_status: pass`, `cpu_comparison_status: pass`,
`host_device_transfer_status: pass`, `failure_stage: none`,
`kernel_proof_wrapper_status: pass`, and `wrapper_exit_status: 0`.

Runtime shell hardware memory-transfer command:
...
Observed 2026-08-19: command exited `0` and wrote
`logs/c1-runner-transfer-proof-2026-08-19T11:49:21Z.log` with
`producer_kind: hardware_memory_transfer`, `transfer_byte_count: 20480`,
`transfer_chunk_count: 5`, `transfer_chunks_completed: 5`,
`transfer_chunk_size_bytes: 4096`, `allocation_total_bytes: 12288`,
`upload_total_bytes: 20480`, `download_total_bytes: 20480`,
`streaming_required: yes`, `sdma_h2d_status: pass`, `sdma_d2h_status: pass`,
`cpu_comparison_status: pass`, `host_device_transfer_status: pass`,
`failure_stage: none`, `transfer_proof_wrapper_status: pass`, and
`wrapper_exit_status: 0`.

### C1R streaming transfer bridge build command

The direct bridge is a separate executable from the runtime runner. Its source
list compiles the CLI adapter with the AMDev lifecycle owner:

```sh
cd ${HOME}/Development/ml/tools/egpu/.worktrees/native-r9700-producer
mkdir -p build/native-r9700-runtime
xcrun --sdk macosx clang++ -std=c++17 -O2 -Wall -Wextra \
  native_r9700/c1_transfer_bridge.cpp native_r9700/amdev_session.cpp native_r9700/amdev_packets.cpp native_r9700/kernel_catalog.cpp -I native_r9700 \
  -o build/native-r9700-runtime/c1_transfer_bridge
```

Runtime shell fp32-sized hardware memory-transfer command:

```sh
cd ${HOME}/Development/ml/tools/egpu/.worktrees/native-r9700-producer
build/native-r9700-runtime/native_r9700_runner --transfer-proof --bytes 40960
```

Observed 2026-08-19: command exited `0` and wrote
`logs/c1-runner-transfer-proof-2026-08-19T11:49:29Z.log` with
`producer_kind: hardware_memory_transfer`, `transfer_byte_count: 40960`,
`transfer_chunk_count: 10`, `transfer_chunks_completed: 10`,
`transfer_chunk_size_bytes: 4096`, `allocation_total_bytes: 12288`,
`upload_total_bytes: 40960`, `download_total_bytes: 40960`,
`streaming_required: yes`, `sdma_h2d_status: pass`, `sdma_d2h_status: pass`,
`cpu_comparison_status: pass`, `host_device_transfer_status: pass`,
`failure_stage: none`, `transfer_proof_wrapper_status: pass`, and
`wrapper_exit_status: 0`.

C0 probe regression command:
...
Observed 2026-08-19: `23 passed in 32.26s`.

Focused runner contract tests (compile the runner, exercise `--lifecycle-dry-run`,
and verify the no-hardware `--kernel-proof`/`--transfer-proof` wrapper seams):
...
Observed 2026-08-19 after C1R-5 primitive hash/review fixes: `18 passed in 17.40s`.
